...
import { getMemberById, setMemberPin, updateMember, phoneExistsForOther } from "~/lib/db.server";
...
export async function action({ request, context }: ActionFunctionArgs) {
  ...
  // Rate limit profile changes: 3 per hour per member
  // Applies to edit-profile and change-pin (both mutate sensitive fields)
  if (intent === "edit-profile" || intent === "change-pin") {
    const rl = await checkRateLimit(DB, `profile:member:${session.memberId}`, 3, 3600);
    if (!rl.allowed) {
      const key = intent === "edit-profile" ? "profileError" : "pinError";
      return json({ [key]: "Too many changes. Limit: 3 per hour. Try again later." });
    }
  }

  if (intent==="edit-profile") {
    const newPhone = ((form.get("phone") as string)||"").trim();
    if (newPhone && !/^\d{10}$/.test(newPhone)) return json({profileError:"Phone must be exactly 10 digits."});
    if (newPhone) {
      const taken = await phoneExistsForOther(DB, newPhone, session.memberId);
      if (taken) return json({profileError:"This phone number is already assigned to another member."});
    }
...
export default function ProfilePage() {
  ...
  return (
    <>
          ...
          <div style={{padding:"16px"}}>
            ...
            {editing?(
              <Form method="post" style={{display:"flex",flexDirection:"column",gap:"14px"}}>
                ...
                <button type="submit" className="btn btn-primary btn-md btn-full" disabled={submitting}>{submitting?"Saving…":"Save Changes"}</button>
              </Form>
            ):(
      ...
      <Toast message={(actionData as any)?.profileError} type="error" />
      <Toast message={(actionData as any)?.profileSuccess?"✅ Profile updated.":undefined} type="success" />
    </>
  );
}
